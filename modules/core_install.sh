...
update_system() {
    ...
    # Добавляем lsb-release только если он должен быть установлен
...
create_synapse_config() {
    ...
    # Определение домена Element Web (по умолчанию используем основной домен)
    if [ -f "$CONFIG_DIR/element_domain" ]; then
        ELEMENT_DOMAIN=$(cat "$CONFIG_DIR/element_domain")
        log "INFO" "Найден домен Element Web: $ELEMENT_DOMAIN"
    else
        ELEMENT_DOMAIN="$MATRIX_DOMAIN"
        log "INFO" "Используем основной домен для Element Web: $ELEMENT_DOMAIN"
    fi
    
    # Определение типа сервера (если не установлен)
    if [ -z "$SERVER_TYPE" ]; then
        if [ -f "$CONFIG_DIR/server_type.conf" ]; then
            source "$CONFIG_DIR/server_type.conf"
            log "INFO" "Загружен тип сервера из конфигурации: $SERVER_TYPE"
        else
            # По умолчанию считаем облачным сервером
            SERVER_TYPE="cloud_server"
            echo "SERVER_TYPE=\"$SERVER_TYPE\"" > "$CONFIG_DIR/server_type.conf"
            log "INFO" "Установлен тип сервера по умолчанию: $SERVER_TYPE"
        fi
    fi
...
# Generated by Matrix Setup Tool v2.0.2
...
# Секреты безопасности (КРИТИЧЕСКИ ВАЖНО - НЕ УДАЛЯЙТЕ!)
...
# Подавление предупреждения о trusted_key_servers
...
EOF
    ...
    # ПРАВИЛЬНАЯ генерация ключа подписи согласно официальной документации
    log "INFO" "Генерация ключа подписи сервера..."
    
    # Устанавливаем правильные права ПОСЛЕ создания файлов конфигурации
    ...
    # Генерация ключей через ПРАВИЛЬНУЮ утилиту Synapse
    # Это ЕДИНСТВЕННЫЙ правильный способ генерации ключа подписи!
    log "INFO" "Генерация ключа подписи через официальную утилиту Synapse..."
    
    # Временно устанавливаем рабочую директорию в директорию с правами записи
    local original_dir=$(pwd)
    cd /tmp
    
    # Пробуем различные способы запуска генератора ключей
    local key_generated=false
    
    # Способ 1: через python3 модуль (наиболее надежный)
    if command -v python3 >/dev/null 2>&1 && python3 -c "import synapse" >/dev/null 2>&1; then
        log "INFO" "Используем системный python3 для генерации ключей..."
        if sudo -u matrix-synapse python3 -m synapse.app.homeserver \
            ...
            --generate-keys >/dev/null 2>&1; then
            log "SUCCESS" "Ключи сгенерированы через системный python3"
            key_generated=true
        fi
    fi
    
    # Способ 2: через virtual environment (если первый не сработал)
    if [ "$key_generated" = false ] && [ -x "/opt/venvs/matrix-synapse/bin/python" ]; then
        log "INFO" "Используем virtual environment для генерации ключей..."
        if sudo -u matrix-synapse /opt/venvs/matrix-synapse/bin/python -m synapse.app.homeserver \
            --server-name="$MATRIX_DOMAIN" \
            --config-path="$SYNAPSE_CONFIG_DIR/homeserver.yaml" \
            --generate-keys >/dev/null 2>&1; then
            log "SUCCESS" "Ключи сгенерированы через virtual environment"
            key_generated=true
        fi
    fi
    
    # Способ 3: через synctl если доступен
    if [ "$key_generated" = false ] && command -v synctl >/dev/null 2>&1; then
        log "INFO" "Используем synctl для генерации ключей..."
        if sudo -u matrix-synapse synctl \
            --config-path="$SYNAPSE_CONFIG_DIR/homeserver.yaml" \
            --generate-keys >/dev/null 2>&1; then
            log "SUCCESS" "Ключи сгенерированы через synctl"
            key_generated=true
        fi
    fi
    ...
    if [ "$key_generated" = false ] || [ ! -f "$signing_key_file" ]; then
        log "ERROR" "Не удалось сгенерировать ключ подписи стандартными методами"
        log "ERROR" "Ключ подписи - критически важный компонент безопасности Matrix"
        log "ERROR" "Его НЕЛЬЗЯ генерировать альтернативными способами!"
        
        echo
        safe_echo "${RED}❌ КРИТИЧЕСКАЯ ОШИБКА: Не удалось сгенерировать ключ подписи${NC}"
        safe_echo "${YELLOW}💡 Возможные причины и решения:${NC}"
        safe_echo "1. ${CYAN}Проблемы с правами доступа:${NC}"
        safe_echo "   sudo chown -R matrix-synapse:matrix-synapse /etc/matrix-synapse/"
        safe_echo "   sudo chown -R matrix-synapse:matrix-synapse /var/lib/matrix-synapse/"
        safe_echo ""
        safe_echo "2. ${CYAN}Неполная установка Synapse:${NC}"
        safe_echo "   sudo apt reinstall matrix-synapse-py3"
        safe_echo ""
        safe_echo "3. ${CYAN}Ручная генерация ключей:${NC}"
        safe_echo "   sudo -u matrix-synapse python3 -m synapse.app.homeserver \\"
        safe_echo "     --server-name=\"$MATRIX_DOMAIN\" \\"
        safe_echo "     --config-path=\"$SYNAPSE_CONFIG_DIR/homeserver.yaml\" \\"
        safe_echo "     --generate-keys"
        safe_echo ""
        safe_echo "4. ${CYAN}Проверка установки Python модулей:${NC}"
        safe_echo "   python3 -c \"import synapse; print('Synapse module OK')\""
        safe_echo ""
        safe_echo "${RED}ВНИМАНИЕ: НЕ используйте OpenSSL или другие способы для генерации${NC}"
        safe_echo "${RED}ключа подписи - это нарушает безопасность Matrix сервера!${NC}"
        
        return 1
    fi
    
    # Проверяем, что ключ был правильно сгенерирован
    if [ ! -s "$signing_key_file" ]; then
        log "ERROR" "Файл ключа подписи пуст или поврежден"
        return 1
    fi
    
    # Проверяем формат ключа (должен содержать "ed25519")
    if ! grep -q "ed25519" "$signing_key_file"; then
        log "ERROR" "Ключ подписи имеет неправильный формат"
        return 1
    fi
    ...
    log "SUCCESS" "Ключ подписи успешно сгенерирован: $signing_key_file"
